...
<head>
	...
	<link rel="stylesheet" type="text/css" href="CSS/DTM_3.css">	
...
<body>
	...
	<section id="content">
		...
		<main>
			...
			<div id="emp_insert_modal" class="insert-cont" style="display: none;">
				<div class="insert-modal">
					...
					<form action="" method="POST" enctype="multipart/form-data">
			            ...
			            <div class="insert-middle">		
			             	 ...
			             	 <div>
								<input type="text" name="name" placeholder="----" required="" autocomplete="off" style="margin-bottom: 23px;">
							 </div>
							 ...
							 <div>
								...
								<div>
									<select name="user_type" style="margin-bottom: 23px;" required>
										...
										<option style="display: none;" value="">--SELECT USER TYPE--</option>  
							 ...
							 <div>
	             	 		 ...
	             	 		 <div style="display: flex; align-items: center;">
								<input type="text" name="username" placeholder="----" required="" autocomplete="off" style="margin-bottom: 23px;" class="inp_user_class">
							 ...
							 <div>
	             	 			<label>Password</label><span class="additional-span">( REQUIRED )</span>
	             	 		 </div>
	             	 		 <div style="display: flex; align-items: center;">
								<input type="text" name="password" placeholder="----" required="" autocomplete="off" style="margin-bottom: 23px;" class="inp_pass_class">
								<span style="background-color: #B0BCCB; color: #fff; border-radius: 4px; padding: 12px 13.5px; font-size: 13px; margin-bottom: 10px; margin-left: 12px; cursor: pointer;" class="generate_pass_class">GENERATE</span>
							 </div>
				...
				<div id="emp_update_modal" class="update-cont" style="display: none;">
					<div class="update-modal">
						...
						<form action="DTM_Controls/DTM_users.php" method="POST" enctype="multipart/form-data">
				...
				<div id="emp_delete_modal" class="delete-cont">
					<div class="delete-modal">
						...
						<form action="DTM_Controls/DTM_PERSONNELS.php" method="POST">
			...
			<div class="head-title" style="margin-bottom: 35px;">
				<div class="left">
					<h1 class="paragraph" style="letter-spacing: -.1px">Personnels</h1>
					<div class="loc-date">
						<ul class="header-main">
							<li style="margin-top: 6px;">
								<i class="fi fi-rr-users" style="color:#C30000; margin-left: 15px;"></i>
							</li>
							<li>
							...
							<li style="margin-top: 2px">
								<a href="#" class="active" style="color: #FFA6A6; margin-left: 10px; color: #585563; font-size: 14px;">Users</a>
							</li>
						</ul>
					</div>
				</div>
			...
			?>
				...
				<div class="add-user-container">
					...
					<div class="add-btns btn-insert" style="margin-left: 10px">
						...
						<a href="DTM_Personnels_CREATE.php" style="padding-bottom: 2px"><button>ADD PERSONNEL</button></a>
					</div>
						...
						<div class="table-header" id="header">
							...
							<span class="id-count-class">
								...
								<?php  
									...
									$sql = "SELECT `ID` FROM `main_i_personal_info` WHERE `approval` = 'APPROVED' ORDER BY `ID`";
									$result=mysqli_query($con,$sql);
							...
							<span id="teach-full-btn" style="margin-left: 0px; font-weight: 600; font-size: 14px;" class="proj-data">All Personnels</span>
					 ...
					 <table id="example" class="table table-striped" style="width:100%">
				        ...
				        <thead>
				            <tr>
				                ...
				                <th class="name-border">NAME</th>
				                <th class="name-border">ACADEMIC/ADMINISTRATIVE RANK</th>
				                <th class="name-border">POSITION</th>
				                <th class="name-border">DEPARTMENT</th>
				                <th class="name-border">CAMPUS</th>
				                <th class="name-border" style="text-align: center!important;">Status</th>
				        ...
				        <tbody>
											...
											$sql="SELECT * FROM `main_i_personal_info` WHERE `approval` = 'APPROVED'";
											$result=mysqli_query($con,$sql);
											if($result){
													$no = 1;
													while($row=mysqli_fetch_assoc($result)){
														$user_id=$row['ID'];
														$Lname=$row['personal_surname'];
														$Fname=$row['personal_firstname'];
														$Mname=$row['personal_middlename'];
														$personal_designation=$row['personal_designation'];
														$academic_rank=$row['personal_academic_rank'];
														$personal_department=$row['personal_department'];
														$campus_name=$row['personal_campus'];
														$personal_emp_status=$row['personal_emp_status'];

														$datecreated_id=$row['date_created'];
												...
												echo
												 ...
												 '<tr>
														...
														<td class="name-border td-flex" style="text-align: left; white-space: nowrap; text-transform: uppercase;">'.$Lname.' '.$Fname.' '.$Mname.'</td>
														<td class="name-border td-flex" style="text-align: left; white-space: nowrap; text-transform: uppercase;">'.$academic_rank.'</td>
														<td class="name-border td-flex" style="text-align: left; white-space: nowrap; text-transform: uppercase;">'.$personal_designation.'</td>
														<td class="name-border td-flex" style="text-align: left; white-space: nowrap; text-transform: uppercase;">'.$personal_department.'</td>
														<td class="name-border td-flex" style="text-align: left; white-space: nowrap; text-transform: uppercase;">'.$campus_name.'</td>
														<td class="name-border td-flex" style="text-align: left; white-space: nowrap; text-transform: uppercase; text-align: center!important;"><span style="background: #7EF35C; padding: 5px 15px; color: #fff; border-radius: 50px;">'.$personal_emp_status.'</span></td>
															...
															<div style="position: relative;">
																...
																<div style="" class="action-cont" id="action-cont-id">
																	<div style="text-align: left;">
																		<a href="DTM_Personnels_VIEW_DETAILS.php?&datecreated_id='.$datecreated_id.'"><button data-id="1" class="btn-update">View Details</button></a>
																	</div>
																	<div style="text-align: left;">
																		<a href="DTM_Personnels_VIEW_PRINT.php?&datecreated_id='.$datecreated_id.'"><button data-id="1" class="btn-update">Print</button></a>
																	</div>
																	<div style="text-align: left;">
																		<a href="DTM_Personnels__IMPENDING_EDIT_INITIAL.php?&datecreated_id='.$datecreated_id.'"><button data-id="1" class="btn-update">Edit</button></a>
																	</div>
																	<div style="text-align: left;">
																		<button data-id="2" class="btn-delete" id="'.$datecreated_id.'">Delete</button>
																	</div>
																</div>
															</div>
	...
	<script>
	    ...
	    $(document).ready(function() {
	      ...
	      $(".generate_pass_class").on("click", function() {

	      	var randomstring = Math.random().toString(36).slice(-8);

	      	$('.inp_pass_class').val(randomstring);
	...
	$(document).ready(function(){
	 ...
	 $(document).on("click", '.btn-delete', function(){
          ...
          $.ajax({
               url:"DTM_Controls/DTM_PERSONNELS.php",
               method:"post",
