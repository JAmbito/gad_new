...
class PersonnelController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->action === "save") {
            Personnel::create($request->except(['action', 'id']));
            $this->func->setLog("Personnel Added", "Inserted", '"'.$request->firstname." ".$request->lastname."\" was added at Personnel Record");
        }

        else {
            Personnel::find($request->id)->update($request->except(['action', 'id']));

            if(PersonnelFamily::where('personnel_id', $request->personnel_id)->exists()) {
                PersonnelFamily::find($request->id)->update($request->except(['action', 'id']));
            } else {
                PersonnelFamily::create($request->except(['action', 'id']));
            }

            if(PersonnelQuestionaire::where('personnel_id', $request->personnel_id)->exists()) {
                PersonnelQuestionaire::find($request->id)->update($request->except(['action', 'id']));
            } else {
                PersonnelQuestionaire::create($request->except(['action', 'id']));
            }

            if(PersonnelReference::where('personnel_id', $request->personnel_id)->exists()) {
                PersonnelReference::find($request->id)->update($request->except(['action', 'id']));
            } else {
                PersonnelReference::create($request->except(['action', 'id']));
            }

            if(PersonnelGovernmentIssued::where('personnel_id', $request->personnel_id)->exists()) {
                PersonnelGovernmentIssued::find($request->id)->update($request->except(['action', 'id']));
            } else {
                PersonnelGovernmentIssued::create($request->except(['action', 'id']));
            }

            foreach ($request->children_records as $key => $children) {
                if($children['children_id'] == null) {
                    $children_data = new PersonnelChildren;
                    $children_data->personnel_id = $request->personnel_id;
            ...
            foreach ($request->educational_records as $key => $educational) {
                if($educational['education_id'] == null) {
                    $educational_data = new PersonnelEducational;
                    $educational_data->personnel_id = $request->personnel_id;
            ...
            foreach ($request->service_records as $key => $service) {
                if($service['service_id'] == null) {
                    ...
                    $service_data->personnel_id = $request->personnel_id;
            ...
            foreach ($request->work_records as $key => $work) {
                if($work['work_id'] == null) {
                    ...
                    $work_data->personnel_id = $request->personnel_id;
            ...
            foreach ($request->voluntary_records as $key => $voluntary) {
                if($voluntary['voluntary_id'] == null) {
                    $voluntary_data = new PersonnelVoluntaryWork;
                    $voluntary_data->personnel_id = $request->personnel_id;
            ...
            foreach ($request->learning_records as $key => $learning) {
                if($learning['learning_id'] == null) {
                    $learning_data = new PersonnelLearning;
                    $learning_data->personnel_id = $request->personnel_id;
            ...
            foreach ($request->hobby_records as $key => $hobby) {
                if($hobby['hobby_id'] == null) {
                    ...
                    $hobby_data->personnel_id = $request->personnel_id;
            ...
            foreach ($request->academic_records as $key => $academic) {
                if($academic['academic_id'] == null) {
                    $academic_data = new PersonnelNonAcademic;
                    $academic_data->personnel_id = $request->personnel_id;
            ...
            foreach ($request->membership_records as $key => $membership) {
                if($membership['membership_id'] == null) {
                    ...
                    $membership_data->personnel_id = $request->personnel_id;
            ...
            $this->func->setLog("Personnel Updated", "Updated", "Personnel Record was updated");
        }
    ...
    public function get() {
        if(request()->ajax()) {
            return datatables()->of(
                Personnel::orderBy('id', 'desc')->with('academic_rank', 'administrative_rank', 'designation', 'department', 'campus')->get()
            )
            ->addIndexColumn()
            ->make(true);
        }
    ...
    {
        $personnel = Personnel::where('id', $id)->with('family', 'question', 'reference', 'government', 'children',
                                'educational', 'service', 'work', 'voluntary', 'learning', 'hobby', 'academic', 'membership')->orderBy('id')->firstOrFail();
        return response()->json(compact('personnel'));
    }
    ...
    public function destroy($id)
    {
        ...
        $destroy->delete();
        $this->func->setLog("Personnel Deleted", "Deleted", 'Record id "'.$id."\" was deleted at Personnel Record");
    ...
    public function save_status(Request $request) {
        $personnel = Personnel::where('id', $request->id)->update(['status' => $request->status]);
        $this->func->setLog("Personnel Status Updated", "Update", 'Personnel Status was updated to "'.$request->status."\" at Personnel Record");
        return 'Record Saved';
    }
}
