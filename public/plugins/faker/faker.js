    ...
    $.fn.fakify = function (options) {
        return this.each(function () {
            ...
            $(this).find('input:text[type!=hidden]').each(function () {
                if ($(this).attr('type') !== 'hidden' && ($.inArray(formatName(this.name), excludeOption) < 0) && ($.inArray(this.name, specifiedOption) < 0)) {
            ...
            $(this).find('input[type=date]').each(function () {
                ...
                const d = randomDate(new Date(2012, 0, 1), new Date(2019, 12, 31));
                const day = d.getDay().toString().length === 1 ? `0${d.getDay().toString()}` : d.getDay().toString();
                const m = (parseInt(d.getMonth().toString()) + 1).toString();
                const month = m.length === 1 ? `0${m}` : m;
                $(this).val(`${d.getFullYear().toString()}-${month}-${day}`);
            });
